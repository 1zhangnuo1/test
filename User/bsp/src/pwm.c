...
*	形    参: _ulFreq : PWM信号频率，单位Hz  (实际测试，最大输出频率为 72M / 4 = 18M）. 0 表示禁止输出
	 ...
	 uint32_t _ulFreq, uint32_t _ulDutyCycle)
{
	...
	if (_ulDutyCycle == 0)           //如果输出为0，直接关闭pwm输出，将gpio设置为推挽输出
	{		
	...
	else if (_ulDutyCycle == 10000)    //设置为输出1
	{
		TIM_Cmd(TIMx, DISABLE);		/* 关闭PWM输出 */
	...
	TIM_TimeBaseStructure.TIM_Period = _ulDutyCycle ;				//自动重装载周期值
	TIM_TimeBaseStructure.TIM_Prescaler = _ulFreq;			//分频系数
